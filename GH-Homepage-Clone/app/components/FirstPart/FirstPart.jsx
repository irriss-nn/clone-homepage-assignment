...
const FirstPart = () => {
    ...
    const handleStickyAndFixed = (topCoord) => {
        setIsSticky(topCoord < 0);
        setIsFixed(topCoord < -20);
    };

    const handleNavState = (topValue, setActiveFn, resetFn1, resetFn2) => {
        if (topValue && topValue < 10) {
            setActiveFn(true);
            resetFn1(false);
            resetFn2(false);
        }
    ...
    useEffect(() => {
        const handleScroll = () => {
            const getTopValue = id => document.getElementById(id)?.getBoundingClientRect().top;

            const topCoordinate = getTopValue('nav');
            handleStickyAndFixed(topCoordinate);

            handleNavState(getTopValue('productivity'), setProduct, setCollab, setSecurity);
            handleNavState(getTopValue('collaboration'), setCollab, setProduct, setSecurity);
            handleNavState(getTopValue('security'), setSecurity, setProduct, setCollab);
        };
